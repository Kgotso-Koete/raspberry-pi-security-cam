...
def home():
    return render_template(
        "camerasettings.html",
        title="Picamera2 WebUI Lite",
        live_settings=live_settings,
        rotation_settings=rotation_settings,
        settings_from_camera=default_settings,
...
def camera_info():
    ...
    connected_camera_data = next(
        (
            module
            for module in camera_module_info["camera_modules"]
            if module["sensor_model"] == connected_camera
        ),
        None,
    )
    if connected_camera_data:
        return render_template(
            "camera_info.html",
            title="Camera Info",
            connected_camera_data=connected_camera_data,
        ...
        for key in data:
            if key in live_settings:
                ...
                if key in ("Resolution"):
                    ...
                    resolution = capture_settings["available-resolutions"][
                        selected_resolution
                    ]
                    stop_camera_stream()
...
def update_restart_settings():
    ...
    try:
        ...
        return jsonify(
            success=True,
            message="Restart settings updated successfully",
            settings=live_settings,
        )
    except Exception as e:
        return jsonify(success=False, message=str(e))
...
def image_gallery():
    try:
        ...
        for image_file in image_files:
            ...
            timestamp = datetime.utcfromtimestamp(unix_timestamp).strftime(
                "%Y-%m-%d %H:%M:%S"
            )
        ...
        return render_template(
            "image_gallery.html",
            image_files=files_and_timestamps_page,
            page=page,
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        level=logging.INFO
    )  # Change the level to DEBUG for more detailed logging
